...
The default window is `midnight`, which captures today's committed progress. Use a different git time window when needed:

```bash
python scripts/daily_linkedin_agent.py --since "yesterday"
```
...
```cron
0 18 * * * cd /mnt/d/Projects/llm-zoomcamp-2026-codesteve && .venv/bin/python scripts/daily_linkedin_agent.py --since midnight
```

This creates a draft each day at 18:00 local machine time.
